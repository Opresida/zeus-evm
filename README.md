...
**🆕 2026-06-25 parte 3 (detalhes em `CLAUDE.md`):** **Painel ZEUS Command com login MAZARI** (deployado na Vercel) — **Supabase Auth** + cadastro por **indicação** (só admin gera) + **aprovação do admin**; membro só vê, **armar o bot = admin-only** (UI + servidor). Branding: **logo oficial ZEUS FLASHLOAN** + **app icon** PWA + favicon. UX: **ZeusLoader** + **splash ≥4s** + **crossfade** + **botão Sair** + **selo de MODO real** (DRY-RUN/ARMADO/LIVE). DRY_RUN liga **subindo a VM** (não por botão); o toggle arma o **live**. Guia `frontend/AUTH_SETUP.md`.
